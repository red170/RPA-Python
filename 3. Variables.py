#Sirven para almacenar Informacion

#Declaracion y Definicion
nombre = 'Oscar Ernesto Portillo'
apellido ='Lopez'
numero = 1

#Modificacion de Variable String
nombre = 'Shulk'

#Modificacion de Variable Numerica
numero += 2 #(El signo previo al igual de la Definicion indica que se le hara al valor original sea suma resta multiplicacion o division con el nuevo operando+)

#Concatenar Strings
nombreCompleto = nombre + apellido

#f-Strings (Template Strings)
saludo = f'hola soy {nombre} y tengo {numero} espada'

#Elim¡nar una Variable (Toma en cuenta la secuencia de ejecucion si se elimina una variable que despues es incluida o usada en otra variable se elimina la variable orginal pero no el resultado de la otra variable)
del apellido

# print(nombre)
# print(numero)
# print(nombreCompleto)
# print(saludo)

#--------- Operadores de Pertenencia & Identidad

print( "ola" in "bienvenida" )
print( "ola" not in "bienvenida" )